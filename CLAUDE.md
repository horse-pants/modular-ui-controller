...
- **Display**: 320×480 touch screen driven by LovyanGFX + LVGL v9. Three tabs — Colour (wheel, horizontal brightness, White/VU pills, effect bar), Effects (`EffectGrid`: a tile per animation, split patterns / audio-reactive), VU. The tabview swipes between them.
- **LEDs**: WS2812 multi-strip via `led_strip` (IDF 5 RMT + DMA), audio-reactive animations
- **Web UI**: AsyncWebServer + WebSocket, Svelte bundle embedded in the firmware image
- **OTA**: firmware only, via `ESP32WifiSetup`. `OTAManager::begin()` is passed a **blank**
  partition label, which makes the library hide the filesystem option on `/update` and refuse
  any filesystem upload — there is no data partition to update.
- **Partitions** (`partitions.csv`): `nvs`, `otadata`, `app0`, `app1`. No data partition; the
  3 MB spiffs used to hold went to the app slots, so each is now 4 MB (~44% used).
...
include/                — headers
  ...
  WhiteButton.h, EffectGrid.h, BrightnessSlider.h
                        — LVGL component classes used by UIManager
                          (ColourWheel paints an HSV disc into an lv_canvas —
                          LVGL 9 has no colour picker widget)
  diag/                 — ScreenMirror, PngStream, ScreenshotRoute
                          (GET /screenshot.png, for help docs)
src/                    — implementation (mirrors include/ tree)
  ui/                   — LVGL component .cpp files + UIManager.cpp
  diag/                 — screenshot capture
  main.cpp              — setup() + loop(); delegation-only
web_src/                — Svelte sources (Vite); builds to web_src/dist
src/generated/          — WebAssets.cpp, the gzipped bundle as flash arrays (git-ignored)
build_web.py            — pre-build hook: vite build → gzip → src/generated/WebAssets.cpp
partitions.csv          — nvs / otadata / app0 / app1; no data partition
platformio.ini          — env: esp32-s3-devkitc-1
```

## Web UI

- **Main control surface** is a Svelte app under `web_src/`, **linked into the firmware image** — not a filesystem. `build_web.py` runs vite, gzips the output and writes it as `const` arrays into `src/generated/WebAssets.cpp`, which lands in flash `.rodata`. `WebApi` serves it with `AsyncProgmemResponse`, which streams straight from the pointer with no copy. There is no LittleFS and no `uploadfs` step: `pio run -t upload` carries the UI with it.
- **Build trigger**: `build_web.py` runs eagerly on every build, but skips npm/vite unless something under `web_src/` is newer than the generated file. `src/generated/` is git-ignored and rebuilt from source.
...
- **Screenshots**: `GET /screenshot.png` returns the current screen as a PNG, for help docs — with a *Screenshot* card on the Settings view (`ScreenshotPanel.svelte`) that previews and downloads it. The panel fetches into a blob rather than pointing an `<img>` at the route, because the device serves one screenshot at a time. LVGL renders partially, so there's no framebuffer to grab — `ScreenMirror` is fed from the flush callback instead and holds a full frame in PSRAM. The PNG is deflated by the S3 ROM's miniz (`tdefl_compress`/`mz_crc32` are ROM exports, so no library is needed; the S3 has no hardware JPEG encoder and the bundled `esp_jpeg` is decode-only) and streamed 40 rows at a time so the async task never stalls.
